...
// ترتیب و برچسب تب‌ها: "همه" اول (پیش‌فرض)، بقیه بعدش
const STATUS_TABS = [
  { value: "ALL", label: "همه" },
...
export default function AdminSupplierRequestsPage() {
  ...
  const fetchRequests = async (status = activeStatus) => {
    try {
      setLoading(true);
      setError("");
      ...
      const res = await fetch(
        `${API}/supplier-requests/admin?${params.toString()}`,
        { credentials: "include" }
      );
  ...
  const handleStatusChange = async (id, status) => {
    try {
      ...
      fetchRequests(activeStatus); // لیست با فیلتر فعلی دوباره لود می‌شود
    } catch (err) {
